...
const EXAMPLE_HEADERS =
  "Asset_ID,Asset_Name,Asset_Type,Business_Owner,Technical_Owner,Criticality,Data_Classification,Hosting,Region,Key_Users_Customers,RTO_hours,RPO_hours,Backup_Enabled,Encryption_Enabled,MFA_Enabled,Logging_Enabled,EDR_Enabled,Vuln_Scanning_Enabled,Notes";
...
const parseAssetRows = (rawText: string) => {
  ...
  if (!hasRequiredHeaders) {
    throw new Error(
      "Заавал байх багана дутуу байна. asset_name, criticality, asset_type эсвэл asset_type_id багануудыг оруулна уу.",
    );
  }
...
function ImportAssetsModal({ onImported }: ImportAssetsModalProps) {
  ...
  const parsedCount = useMemo(() => {
    ...
    try {
      return parseAssetRows(rawText).length;
    } catch {
      return 0;
    }
  }, [rawText]);
