...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!hasRecommendations || !hasScfControls) {
      return NextResponse.json({ recommendations: [], count: 0 });
    }

    let query = `
        SELECT cr.*, sc.scf_domain, sc.scf_control_name, sc.scf_description
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hasScfControls) {
      return NextResponse.json({
        message:
          "SCF control table is not available; use NIST CSF control objectives instead.",
        count: 0,
        recommendations: [],
        context: {
          riskLevel: riskLevel,
    ...
    for (const control of controls) {
      if (seenControlIds.has(control.scf_control_id)) continue;
      seenControlIds.add(control.scf_control_id);

      // Check if recommendation already exists
      const existingResult = await pool.query(
        `SELECT id FROM control_recommendations
         WHERE risk_analysis_id = $1 AND scf_control_id = $2`,
        [risk_analysis_id, control.scf_control_id],
      );
