...
async function ensureControlRecommendationColumns() {
  ...
  const columns = [
    ...
    "ALTER TABLE control_recommendations ADD COLUMN IF NOT EXISTS scf_control_id VARCHAR(50)",
...
async function syncControlRecommendationsToActions() {
  ...
  const hasScfControls = await tableExists("scf_controls");
  ...
  const controlNameExpression = hasScfControls
    ? "COALESCE(cr.control_name, sc.scf_control_name, cr.scf_control_id, 'Recommended control')"
    : "COALESCE(cr.control_name, cr.scf_control_id, 'Recommended control')";
  const categoryExpression = hasScfControls
    ? "COALESCE(cr.nist_csf_category, rr.nist_csf_category, sc.nist_csf_category, cr.nist_function)"
  ...
  const joins = [
    ...
    hasScfControls
      ? "LEFT JOIN scf_controls sc ON sc.scf_control_id = cr.scf_control_id"
      : "LEFT JOIN (SELECT NULL::text AS scf_control_id, NULL::text AS scf_control_name, NULL::text AS nist_csf_category) sc ON false",
  ].join("\n");
