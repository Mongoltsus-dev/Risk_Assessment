...
async function getControls(): Promise<ControlRow[]> {
  ...
  const hasSubcategoryIds = await columnExists("nist_controls", "csf_subcategory_ids");
  const hasIsActive = await columnExists("nist_controls", "is_active");
  const result = await pool.query(`
    SELECT control_id,
           control_name,
           nist_function,
...
async function flattenSubcategories() {
  ...
  return result.rows.map((row) => ({
    ...
    nist_function: FUNCTION_BY_CODE[row.function_code] ?? row.nist_function ?? "Other",
  }));
}
...
async function mergeComplianceRows(input: {
  ...
  return (await flattenSubcategories()).map<ComplianceRow>((sub) => {
    ...
    return {
      ...
      nist_function: assessment?.nist_function ?? sub.nist_function,
