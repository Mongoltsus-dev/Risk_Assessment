...
function toScoreNumber(value?: number | string | null) {
  const score = Number(value ?? 0);
  return Number.isFinite(score) ? score : 0;
}

function getRiskLevelClass(level?: string | null) {
  switch (level) {
    case "Critical":
...
const APPETITE_LEVELS = [
  {
    ...
    desc: "Байгууллагыг эрсдэлийг хүлээж авах хэмжээ маш бага.",
  },
  {
    ...
    ring: "ring-blue-500",
    desc: "Байгууллагын эрсдэлийг хүлээж авах хэмжээ бага.",
  },
  {
    ...
    ring: "ring-amber-500",
    desc: "Байгууллага эрсдэлийг хүлээж авах хэмжээ дундаж.",
  },
  {
    ...
    desc: "Байгууллагын эрсдэлийг хүлээж авах хэмжээ өндөр.",
  },
  {
    ...
    ring: "ring-red-500",
    desc: "Байгууллагын эрсдэлийг хүлээж авах хэмжээ маш өндөр.",
  },
];
...
export default function OrganizationProfilePage() {
  ...
  const fetchBusinessProcesses = async () => {
    try {
      ...
      setBusinessProcessError("Business process жагсаалт ачаалж чадсангүй.");
    }
  };
  ...
  const handleAddBusinessProcess = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(data.error || "Business process бүртгэж чадсангүй.");
      ...
      setBusinessProcessError(
        error instanceof Error
          ? error.message
          : "Business process бүртгэж чадсангүй.",
      );
    } finally {
  ...
  const handleRemoveBusinessProcess = async (
    ...
    if (
      !window.confirm(
        `"${process.process_name}" business process-ийг устгах уу?`,
      )
    )
      return;
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok)
        throw new Error(data.error || "Business process устгаж чадсангүй.");
      ...
      setBusinessProcessError(
        error instanceof Error
          ? error.message
          : "Business process устгаж чадсангүй.",
      );
    } finally {
      setBusinessProcessSaving(false);
    }
            ...
            <CardContent className="space-y-6 pt-6">
              ...
              <div className="rounded-lg border bg-background/50 p-4">
                <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-muted-foreground">
                  ...
                  Байгууллагын чухал business process
                </div>
                {businessProcesses.length > 0 ? (
                  <div className="grid gap-2 sm:grid-cols-2">
                    {businessProcesses.map((process) => {
                      const riskScore = toScoreNumber(
                        process.highest_risk_score,
                      );
                      const linkedRisks = process.linked_risks ?? [];
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="flex shrink-0 flex-wrap justify-end gap-1.5">
                              {typeof process.asset_count === "number" && (
                                <Badge className="border border-blue-500/20 bg-blue-500/10 text-blue-600">
                                  {process.asset_count} asset
                                </Badge>
                              )}
                              {typeof process.risk_count === "number" && (
                                <Badge className="border border-slate-500/20 bg-slate-500/10 text-slate-700 dark:text-slate-300">
                                  {process.risk_count} risk
                                </Badge>
                              )}
                              {riskScore > 0 && (
                                <Badge
                                  className={`border ${getRiskLevelClass(
                                    process.highest_risk_level,
                                  )}`}
                                >
                                  Max {riskScore} ·{" "}
                                  {process.highest_risk_level ?? "Unknown"}
                                </Badge>
                              )}
                            </div>
                          </div>
                            ...
                            <div className="mt-2 space-y-1.5">
                              {linkedRisks.slice(0, 3).map((risk) => (
                                ...
                                >
                                  ...
                                  {toScoreNumber(risk.risk_score) > 0 && (
                                    <span
                                      className={`shrink-0 rounded border px-1.5 py-0.5 font-semibold ${getRiskLevelClass(
                                        risk.risk_level,
                                      )}`}
                                    >
                                      {toScoreNumber(risk.risk_score)}
                                    </span>
                                  )}
                                </div>
                              ))}
                  ...
                  <p className="text-sm text-muted-foreground">
                    Чухал business process бүртгээгүй байна.
                  </p>
                )}
              </div>
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <Card className={`${ORG_PROFILE_CARD_CLASS} overflow-hidden shadow-sm`}>
          <CardHeader className="border-b border-border pb-6">
            ...
            <CardDescription>
              Доорх мэдээллийг оруулснаар таны байгууллагын эрсдэлийн хүлээж
                ...
                <div className={ORG_FORM_PANEL_CLASS}>
                  ...
                  <div className="mt-4 grid gap-2">
                    {businessProcesses.length > 0 ? (
                          ...
                          >
                            <div className="min-w-0">
                              ...
                              <div className="mt-1 flex flex-wrap gap-1.5 text-xs text-muted-foreground">
                                {typeof process.asset_count === "number" && (
                                  <span>
                                    Холбогдсон asset: {process.asset_count}
                                  </span>
                                )}
                                {typeof process.risk_count === "number" && (
                                  <span>Эрсдэл: {process.risk_count}</span>
                                )}
                                {riskScore > 0 && (
                                  <span
                                    className={`rounded border px-1.5 py-0.5 font-semibold ${getRiskLevelClass(
                                      process.highest_risk_level,
                                    )}`}
                                  >
                                    Max score {riskScore}
                            ...
                            <Button
                              ...
                              title="Устгах"
                              aria-label="Business process устгах"
                            >
                      ...
                      <p className="text-sm text-muted-foreground">
                        Одоогоор чухал business process бүртгээгүй байна.
                      </p>
                    )}
                  </div>
                </div>
