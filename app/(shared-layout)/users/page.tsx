...
const STATUS_STYLE: Record<string, string> = {
  active: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300 border-emerald-200 dark:border-emerald-900",
  inactive: "bg-slate-100 text-slate-600 dark:bg-slate-800/50 dark:text-slate-400 border-slate-200 dark:border-slate-700",
  suspended: "bg-red-100 text-red-700 dark:bg-red-950/50 dark:text-red-300 border-red-200 dark:border-red-900",
};
...
export default function UsersPage() {
  ...
  const handleDelete = async (u: AppUser) => {
    if (!window.confirm(`"${u.full_name}" хэрэглэгчийг устгахдаа итгэлтэй байна уу?`)) return;
    try {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 md:p-8">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Нийт хэрэглэгч", value: stats.total, icon: Users, color: "text-blue-600" },
          { label: "Идэвхтэй", value: stats.active, icon: User, color: "text-emerald-600" },
          { label: "Удирдлага", value: stats.managers, icon: Shield, color: "text-amber-600" },
                ...
                {filtered.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <Badge className={`text-xs border ${ROLE_STYLE[u.role_id] ?? "bg-muted text-muted-foreground"}`}>
                    ...
                    {/* Status */}
                    <td className="px-4 py-3 text-center">
                      <Badge className={`text-xs border ${STATUS_STYLE[u.status] ?? "bg-muted text-muted-foreground"}`}>
        ...
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            ...
            <div>
              ...
              />
              {editingUser && (
                <p className="text-xs text-muted-foreground mt-1">Имэйл өөрчлөх боломжгүй</p>
              )}
            </div>

            <div>
              <Label htmlFor="password" className="mb-1.5 block">
                Нууц үг {editingUser ? "(хоосон орхивол өөрчлөгдөхгүй)" : <span className="text-red-500">*</span>}
              </Label>
              <Input
                id="password"
                type="password"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="role_id" className="mb-1.5 block">Дүр</Label>
                <select
                  id="role_id"
              ...
              <div>
                <Label htmlFor="status" className="mb-1.5 block">Статус</Label>
                <select
                  id="status"
            ...
            <DialogFooter className="pt-2 gap-2">
              ...
              <Button type="submit" disabled={saving} className="bg-blue-600 hover:bg-blue-700">
                {saving ? "Хадгалж байна..." : editingUser ? "Өөрчлөлт хадгалах" : "Хэрэглэгч нэмэх"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
